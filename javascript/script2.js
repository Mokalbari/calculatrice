...
const pushOperator = (event) => {
  ...
  if (result === "") {
    ...
    operator = event;
    mainScreenDisplay = [];
  } else if (result !== "") smallScreenDisplay = [result];
  result = "";
  operator = event;
  mainScreenDisplay = [];
};
...
const calculateResult = () => {
  ...
  switch (operator) {
    case "+":
      result = mathFunction.add(smallScreenValue, mainScreenValue);
      if (result !== Math.floor(result)) {
        result = +result.toFixed(2);
      }
      break;
    case "-":
      result = mathFunction.substract(smallScreenValue, mainScreenValue);
      if (result !== Math.floor(result)) {
        result = +result.toFixed(2);
      }
      break;
    case "/":
      if (smallScreenValue === 0 || mainScreenValue === 0) {
        ...
        result = mathFunction.divide(smallScreenValue, mainScreenValue);
        if (result !== Math.floor(result)) {
          result = +result.toFixed(2);
        }
      }
      break;
    case "*":
      result = mathFunction.multiply(smallScreenValue, mainScreenValue);
      if (result !== Math.floor(result)) {
        result = +result.toFixed(2);
      }
      break;
